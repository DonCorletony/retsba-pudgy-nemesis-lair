...
const btn =
  ...
  'shadow-[inset_1px_1px_0_#e6e6e6,inset_-1px_-1px_0_#8a8a8a] px-4 py-1.5 font-bold text-black text-sm ' +
  ...
  <ConnectButton.Custom>
    {({ account, chain, openConnectModal, openChainModal, openAccountModal, authenticationStatus, mounted }) => {
      ...
      if (!connected) {
        return (
          <div className={className}>
            <button onClick={openConnectModal} className={btn}>CONNECT WALLET</button>
          </div>
        );
      }
      ...
      if (chain.unsupported) {
        return (
          <div className={className}>
            <button onClick={openChainModal} className={`${btn} !text-[#a30000]`}>WRONG NETWORK</button>
          </div>
        );
      }
      ...
      return (
        <div className={className}>
          <button onClick={openAccountModal} className={btn} title={account.address}>
            {account.displayName}
          </button>
        </div>
      );
